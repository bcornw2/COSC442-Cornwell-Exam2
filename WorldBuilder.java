...
public class WorldBuilder {
	...
	private WorldBuilder smooth(int times) {
		...
		advanceFloor(times, tiles2);
		return this;
	}

	private void advanceFloor(int times, Tile[][][] tiles2) {
	...
	private boolean ifWidth(int x, int y, int ox, int oy) {
	...
	private int fillRegion(int region, int x, int y, int z) {
		...
		while (!open.isEmpty()){
			...
			size = neighborLoop(region, size, open, p);
		}
		return size;
	}

	private int neighborLoop(int region, int size, ArrayList<Point> open, Point p) {
